  ...
  >
    ...
    >
      ...
      <textarea
        ...
        @keydown.enter.exact.prevent="createClientMessage"
      />
...
export default {
  ...
  emits: ['update:modelValue', 'createClientMessage', 'createClientFile', 'onPaste', 'cantWriteMessages', 'changeHeight'],
  data () {
    return {
      drag: false
    }
  ...
  methods: {
    ...
    createClientMessage () {
      ...
      this.$emit('createClientMessage', this.computedValue)
    },
    cantWriteHandler () {
      if (!this.canAddFiles) {
