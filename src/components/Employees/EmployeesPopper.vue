<template>
  ...
  >
    <div
      ...
      :class="{ 'bg-[#7B94EB] text-white': !responsibleUserUid, 'bg-[#F4F5F7] text-[#575758]': responsibleUserUid, 'cursor-pointer': canEdit }"
    >
      <!-- Empty label for responsible user -->
      <div
        v-if="!responsibleUserUid"
      ...
      <div class="max-h-[156px] max-w-[300px] overflow-y-scroll scroll-style">
        ...
        >
          <div
            ...
            @click="$emit('update:modelValue', employee.uid), close()"
          >
            <img
...
export default {
  ...
  computed: {
    ...
    userName () {
      return this.employees[this.responsibleUserUid.toLowerCase()]?.name || 'Выберите сотрудника'
    },
    userPhoto () {
      return this.employees[this.responsibleUserUid.toLowerCase()]?.fotolink ?? ''
    }
  }
}
</script>
